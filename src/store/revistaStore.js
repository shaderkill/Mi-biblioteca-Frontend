...
export default {
  ...
  actions: {
    ...
    // Obtener todos
    async getAll({ commit }) {
      let response = await global.getAllFrom({ entity: "revistas" });
    ...
    async saveEntity({ dispatch, commit }, item) {
      global.createIn({ entity: "revistas", item: item });
      dispatch("getAll");
      commit("clearEntity");
    },
    // Actualizar objeto
    async updateEntity({ dispatch }, item) {
      await global.updateIn({ entity: "revistas", item: item });
      dispatch("getAll");
    },
    // Deshabilitar objeto
    async disableEntity(item) {
      let id = item.id;
      global.stateOf({ entity: "revistas", id: id });
    },
    // Eliminar Objeto
    async deleteEntity({ commit }, item) {
      await Swal.fire({
        ...
        if (result.value) {
          let id = item.id;
          global.deleteFrom({ entity: "revistas", id: id });
