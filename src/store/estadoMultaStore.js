...
export default {
  ...
  actions: {
    ...
    // Obtener todos
    async getAll({ commit }) {
      let response = await global.getAllFrom({ entity: "estado_multas" });
      commit("setArray", response);
    },
    // Obtener por ID
    async getByID({ commit }, id) {
      let item = await global.getByIDFrom({ entity: "estado_multas", id: id });
      commit("setEntity", item);
    },
    // Guardar objeto
    async saveEntity({ dispatch, commit }, item) {
      global.createIn({ entity: "estado_multas", item: item });
      dispatch("getAll");
      commit("clearEntity");
    },
    // Actualizar objeto
    async updateEntity({ dispatch }, item) {
      await global.updateIn({ entity: "estado_multas", item: item });
      dispatch("getAll");
    },
    // Deshabilitar objeto
    async disableEntity(item) {
      let id = item.id;
      global.stateOf({ entity: "estado_multas", id: id });
    },
    // Eliminar Objeto
    async deleteEntity({ commit }, item) {
      await Swal.fire({
        ...
        if (result.value) {
          let id = item.id;
          global.deleteFrom({ entity: "estado_multas", id: id });
