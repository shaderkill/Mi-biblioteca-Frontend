...
export default {
  ...
  actions: {
    ...
    // Obtener todos
    async getAll({ commit }) {
      let response = await global.getAllFrom({ entity: "proyectos" });
    ...
    async saveEntity({ dispatch, commit }, item) {
      global.createIn({ entity: "proyectos", item: item });
      dispatch("getAll");
      commit("clearEntity");
    },
    // Actualizar objeto
    async updateEntity({ dispatch }, item) {
      await global.updateIn({ entity: "proyectos", item: item });
      dispatch("getAll");
    },
    // Deshabilitar objeto
    async disableEntity(item) {
      let id = item.id;
      global.stateOf({ entity: "proyectos", id: id });
    },
    // Eliminar Objeto
    async deleteEntity({ commit }, item) {
      await Swal.fire({
        ...
        if (result.value) {
          let id = item.id;
          global.deleteFrom({ entity: "proyectos", id: id });
